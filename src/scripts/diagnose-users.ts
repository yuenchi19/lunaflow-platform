...
async function main() {
    ...
    const activeStudents = users.filter(u => {
        ...
        // Also check if subscriptionStatus would be a factor if we could see it
        // But for now, just checking 'status' which is what sync script updates.
        return false;
    });
